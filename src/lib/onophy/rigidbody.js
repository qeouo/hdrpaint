...
export default class RigidBody extends PhyObj{
	...
	refreshInertia(){
		...
		var sx = obj.bound_box.max[0]*obj.scale[0];
		var sy = obj.bound_box.max[1]*obj.scale[1];
		var sz = obj.bound_box.max[2]*obj.scale[2];
			
		//慣性テンソルベースの計算
		Mat33.mul(this.inertiaTensorBase,this.inertiaTensorBase,0);
		Vec3.setValue(this.COMBase,0,0,0 );
		switch(type){
		...
		case Collider.CONE:
			...
			Vec3.setValue(this.COMBase,0,0,-sz*0.5 );
			break;
		...
		case Collider.MESH:
			...
			var S=0; //総体積
			var PS = Vec3.alloc(); //体積*重心
			Vec3.setValue(PS,0,0,0);
			var p = Vec3.alloc(); //部分の重心
			var p1 = Vec3.alloc(); //四面体の頂点(４つ目は原点)
			var p2 = Vec3.alloc();
			var p3 = Vec3.alloc();
			var I = Mat33.alloc(); //慣性モーメント
	...
	refreshCollision(){
		...
		case Collider.CONVEX_HULL:
			...
			var a = Vec4.alloc();
			Vec4.setValue(a,this.rotq[0],-this.rotq[1],-this.rotq[2],-this.rotq[3]);
			Vec4.free(1);
			sx=this.scale[0];
			sy=this.scale[1];
			sz=this.scale[2];
		...
		var scale = Vec3.alloc();
		Vec3.setValue(scale,sx,sy,sz);
